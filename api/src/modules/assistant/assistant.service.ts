...
function buildSystemPrompt(context: AssistantChatContext): string {
  const lines = [
    ...
    'Quando houver alteração de código, prefira responder com um resumo curto seguido de um único bloco ```diff em formato unified diff.',
    'No diff, use caminhos relativos ao workspace e headers padrão com --- e +++.',
    'Só devolva arquivo final completo em bloco de código normal quando o usuário pedir explicitamente o arquivo inteiro.',
